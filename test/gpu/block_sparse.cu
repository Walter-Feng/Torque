...
TEST_CASE("block sparse tensor operation") {
    ...
    SECTION("block sparse matrix initialization") {
        ...
        torque::gpu::BlockSparseTensor<float> tensor(a.data(), arma::uvec{1, 1}, arma::uvec{2, 2}, dimension);
...
//
//        torque::gpu::BlockSparseTensor<float> matrix_in_tensor({2, 2}); // row vector
//
