...
TEST_CASE("CPU vs GPU") {
  ...
  SECTION("large tensor - matrix contraction") {
    ...
    const arma::uvec subblock_table = torque::util::generate_index_table(
        subblock_dimension);
    ...
    for (arma::uword i = 0; i < 3000; i++) {
      ...
      const arma::uvec original_indices = torque::util::index_to_indices(i,
                                                                         subblock_table);

      const arma::uword original_index = arma::sum(
          original_indices % tensor_table);
    ...
    const arma::uvec submatrix_table = torque::util::generate_index_table(
        submatrix_dimension);
    ...
    const torque::SparseTensor<float> cpu_matrix_in_tensor(matrix.data(),
                                                           matrix_indices,
                                                           matrix_table,
                                                           matrix_dimension);
...
#define STOP_RECORD_TIMER(name) {       \
    ...
    const auto dense_cpu_contraction =
        cpu_dense_tensor_format.contract(cpu_dense_matrix_format,
                                         arma::umat{{0, 0},
                                                    {1, 1}});

    STOP_RECORD_TIMER(cpu_time_contraction);

    std::cout << "CPU time (dense contraction): " << cpu_time_contraction
    ...
    const auto cpu_contraction =
        cpu_tensor_format.contract(cpu_matrix_in_tensor, arma::umat{{0, 0},
                                                                    {1, 1}});

    STOP_RECORD_TIMER(cpu_time_contraction);

    std::cout << "CPU time (sparse contraction): " << cpu_time_contraction
              << " milliseconds" << std::endl;

    START_TIMER();

    const auto cpu_block_sparse_contraction =
        cpu_block_sparse_tensor_format.contract(
            cpu_block_sparse_matrix_in_tensor, arma::umat{{0, 0},
                                                          {1, 1}});

    STOP_RECORD_TIMER(cpu_time_contraction);

    std::cout << "CPU time (block sparse contraction): " << cpu_time_contraction
              << " milliseconds" << std::endl;
    ...
    std::cout << "GPU time (sparse contraction): " << gpu_time_contraction
              << " milliseconds" << std::endl;

    for (arma::uword i = 0; i < 10; i++) {
      CHECK(
          std::abs(dense_cpu_contraction.query({i}) - contraction.query({i})) <
          5e-5);
      CHECK(
          std::abs(cpu_contraction.query({i}) - contraction.query({i})) < 5e-5);
      CHECK(std::abs(
          cpu_block_sparse_contraction.query({i}) - contraction.query({i})) <
            5e-5);
    }
  }
}
