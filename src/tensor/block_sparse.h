...
public:
  ...
  explicit BlockSparseTensor(const T * source_data,
    ...
    if (rank > 0) {
      ...
      for (arma::uword i = 0; i < n_blocks; i++) {
        this->index_tables.col(i) = util::generate_index_table(
            this->blocks_dimension.col(i));
      }

      this->data = std::unique_ptr<T>(
          (T *) malloc(sizeof(T) * arma::sum(block_n_elem)));

      if (source_data) {
        memcpy(this->data.get(), source_data,
               sizeof(T) * arma::sum(block_n_elem));
      } else {
  ...
  inline
  explicit BlockSparseTensor(std::unique_ptr<T> && source_data,
                             const arma::umat & begin_points,
                             const arma::umat & end_points,
                             const arma::uvec & dimension) {
      ...
      if (in_range.n_elem) {
        ...
        for (arma::uword i = 0; i < in_range.n_elem; i++) {
          ...
          temp +=
              this->data.get()[block_offsets(block_index) +
                               arma::sum(relative_indices %
                                         this->index_tables.col(block_index))];
        }
  ...
  contract(const BlockSparseTensor<U> & tensor,
    ...
    const arma::uvec this_sort_index = arma::sort_index(
        this_contracting_indices);
    const arma::uvec that_sort_index = arma::sort_index(
        that_contracting_indices);
    const arma::uvec sorted_this_contracting_indices = this_contracting_indices(
        this_sort_index);
    const arma::uvec sorted_that_contracting_indices = that_contracting_indices(
        that_sort_index);
    ...
    for (arma::uword i = 0; i < this->block_n_elem.n_elem; i++) {
      ...
      const arma::uvec A_begin_point_in_contracting_dimension = A_begin_point.rows(
          this_contracting_indices);
      const arma::uvec A_end_point = this->end_points.col(i);

      const auto contracting_info = block_sparse::block_in_range(
          contracting_indices, A_begin_point, A_end_point,
      ...
      const arma::umat A_subblock_dimension =
          A_subblock_end_points - A_subblock_begin_points +
      ...
      const arma::umat B_subblock_dimension =
          B_subblock_end_points - B_subblock_begin_points +
          arma::ones<arma::umat>(arma::size(B_subblock_begin_points));

      for (arma::uword j_block = 0;
           j_block < contracting_info.block_indices.n_elem; j_block++) {
        const arma::uword B_block_index = contracting_info.block_indices(
            j_block);

        const arma::uvec B_begin_point = tensor.begin_points.col(B_block_index);


        const arma::uvec B_begin_point_in_contracting_dimension = B_begin_point.rows(
            that_contracting_indices);

        const arma::uvec j_contracting_start = contracting_info.contraction_begin_points.col(
            j_block);
        const arma::uvec j_contracting_end = contracting_info.contraction_end_points.col(
            j_block);


        const arma::uvec j_contracting_table = contracting_info.contraction_tables.col(
            j_block);
        const arma::uvec j_contract_dim =
            j_contracting_end - j_contracting_start +
            arma::ones<arma::uvec>(j_contracting_start.n_elem);


        if (result.rank > 0) {
          const arma::uvec j_begin_point = contracting_info.new_begin_points.col(
              j_block);
          const arma::uvec j_end_point = contracting_info.new_end_points.col(
              j_block);

          const arma::uvec j_dim = j_end_point - j_begin_point +
                                   arma::ones<arma::uvec>(j_begin_point.n_elem);
          ...
          const std::vector<T> vectorised_data =
              arma::conv_to<std::vector<T>>::from(
                  arma::vectorise(multiplication));
          ...
          for (arma::uword contract_index = 0;
               contract_index < arma::prod(j_contract_dim); contract_index++) {

            const arma::uvec relative_contraction_indices = util::index_to_indices(
                contract_index, j_contract_dim);
            const arma::uvec contraction_indices =
                relative_contraction_indices + j_contracting_start;

            // assign the summation indices to the original tensor indices
            const arma::uvec this_dimension_indices =
                contraction_indices(this_sort_index) - A_begin_point;
            const arma::uvec that_dimension_indices =
                contraction_indices(that_sort_index) - B_begin_point;

            const std::common_type_t<T, U>
                elem = this->data.get()[arma::sum(
                this->index_tables.col(i) % this_dimension_indices)
                                        + this->block_offsets(i)]
                       * tensor.data.get()[
                           arma::sum(tensor.index_tables.col(B_block_index) %
                                     that_dimension_indices)
                           + tensor.block_offsets(B_block_index)];
  ...
  BlockSparseTensor<T> hard_transpose(const arma::uvec & permutation) const {
    ...
    for (arma::uword i = 0; i < new_begin_points.n_cols; i++) {
      const arma::uvec new_table = util::generate_index_table(
          new_blocks_dimension.col(i));
      const arma::uword n_elem = arma::prod(new_blocks_dimension.col(i));

      const arma::uvec old_table_sort_index = arma::sort_index(
          this->index_tables.col(i));

      T * block_data = (T *) malloc(n_elem * sizeof(T));
      for (arma::uword j = 0; j < n_elem; j++) {
        const arma::uvec old_indices = util::index_to_indices(j,
                                                              this->index_tables.col(
                                                                  i),
        ...
        block_data[arma::sum(new_indices % new_table)] =
            this->data.get()[
                arma::sum(old_indices % this->index_tables.col(i)) +
                this->block_offsets(i)];
      }

      result.append_block(block_data, new_begin_points.col(i),
                          new_end_points.col(i), new_table);
    }
