...
public:
  ...
  explicit DenseTensor(const arma::uvec & dimension) {
    ...
    if (rank > 0) {
      ...
      this->data = std::unique_ptr<T>(
          (T *) malloc(sizeof(T) * arma::prod(dimension)));
  ...
  inline
  explicit DenseTensor(std::unique_ptr<T> && source_data,
  ...
  DenseTensor(const DenseTensor & tensor) {
    ...
    this->data = std::unique_ptr<T>(
        (T *) malloc(sizeof(T) * arma::prod(this->dimension)));

    if (tensor.data) {
      memcpy(this->data.get(), tensor.data.get(),
             sizeof(T) * arma::prod(dimension));
    } else {
      throw Error("Source data not allocated!");
    }
  ...
  contract(const DenseTensor<U> & tensor,
    ...
    const arma::uvec contract_dimension = this->dimension(
        this_contracting_indices);
    const arma::uvec contract_table = util::generate_index_table(
        contract_dimension);
    ...
    const arma::uvec new_dimension = arma::join_vert(this_dimension_copy,
                                                     that_dimension_copy);
    const arma::uvec new_dimension_table = util::generate_index_table(
        new_dimension);
    ...
    if (result.rank > 0) {
      for (arma::uword i = 0; i < arma::prod(new_dimension); i++) {
        ...
        const arma::uvec new_dimension_indices = util::index_to_indices(i,
                                                                        new_dimension_table);

        arma::uvec this_dimension_indices = new_dimension_indices.rows(0,
                                                                       this->rank -
                                                                       contract_dimension.n_elem -
                                                                       1);
        arma::uvec that_dimension_indices =
            this->rank - contract_dimension.n_elem <= result.rank - 1 ?
            new_dimension_indices.rows(this->rank - contract_dimension.n_elem,
                                       result.rank - 1) :
            arma::uvec{};

        for (arma::uword k = 0;
             k < sorted_this_contracting_indices.n_elem; k++) {
          this_dimension_indices.insert_rows(sorted_this_contracting_indices(k),
                                             1);
          that_dimension_indices.insert_rows(sorted_that_contracting_indices(k),
                                             1);
        }

        for (arma::uword j = 0; j < arma::prod(contract_dimension); j++) {
          const arma::uvec contraction_indices = util::index_to_indices(j,
                                                                        contract_table);

          // assign the summation indices to the original tensor indices
          for (arma::uword k = 0; k < contraction_indices.n_elem; k++) {
            this_dimension_indices(
                this_contracting_indices(k)) = contraction_indices(k);
            that_dimension_indices(
                that_contracting_indices(k)) = contraction_indices(k);
          }
          ...
          const std::common_type_t<T, U>
              elem = this->query(this_dimension_indices) *
                     tensor.query(that_dimension_indices);
      ...
      for (arma::uword j = 0; j < arma::prod(contract_dimension); j++) {
        ...
        // assign the summation indices to the original tensor indices
        const arma::uvec this_dimension_indices = contraction_indices(
            this_sort_index);
        const arma::uvec that_dimension_indices = contraction_indices(
            that_sort_index);
  ...
  DenseTensor<T> hard_transpose(const arma::uvec & permutation) const {
    ...
    for (arma::uword i = 0; i < total_elem; i++) {
      ...
      const arma::uvec new_indices = util::index_to_indices(i,
                                                            this->index_table,
                                                            sort_index);

      new_data_pointer[arma::sum(
          new_indices(permutation) % new_table)] = data_pointer[i];
    }
