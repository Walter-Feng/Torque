...
public:
  ...
  explicit SparseTensor(const T * source_data,
    ...
    this->dimension = dimension;
    this->indices = thrust::device_vector<int32_t>(
        arma::conv_to<std::vector<T>>::from(indices));
    ...
    if (rank > 0) {
      ...
      if (source_data) {
        thrust::copy(source_data, source_data + indices.n_elem,
                     this->data.begin());
      } else {
  ...
  void initialize(const T * source_data, const arma::uvec & indices) {
    ...
    if (source_data) {
      this->indices = thrust::device_vector<int32_t>(
          arma::conv_to<std::vector<int32_t>>::from(indices));
      thrust::copy(source_data, source_data + indices.n_elem,
                   this->data.begin());
    } else {
      throw Error("Source data not allocated!");
      ...
      if (this->rank == 0) {
        ...
        if (found_index.n_elem) {
          ...
          if (number == 0) {
            ...
            this->indices[found_index(0)] = this->indices[this->indices.size() -
                                                          1];
  ...
  SparseTensor<T>
  contract(cublasHandle_t handle, const SparseTensor<T> & tensor,
    ...
    const arma::uvec contract_table = torque::util::generate_index_table(
        contract_dimension);
    ...
    if constexpr(std::is_same<T, float>::value) {
      cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_T, this->indices.size(),
                  tensor.indices.size(), 1, &one,
    ...
    if constexpr(std::is_same<T, double>::value) {
      cublasDgemm(handle, CUBLAS_OP_N, CUBLAS_OP_T, this->indices.size(),
                  tensor.indices.size(), 1, &one,
                  this_pointer, this->indices.size(),
                  that_pointer, tensor.indices.size(),
                  &zero, out_pointer, this->indices.size());
    ...
    if constexpr(std::is_same<T, half>::value) {
      cublasHgemm(handle, CUBLAS_OP_N, CUBLAS_OP_T, this->indices.size(),
                  tensor.indices.size(), 1, &one,
                  this_pointer, this->indices.size(),
                  that_pointer, tensor.indices.size(),
                  &zero, out_pointer, this->indices.size());
    ...
    thrust::sort_by_key(new_indices_raw.begin(), new_indices_raw.end(),
                        raw_output.begin());
    ...
    return {std::move(reduced_data), std::move(reduced_indices),
            new_dimension_table, new_dimension};
  }
  ...
  SparseTensor<T> hard_transpose(const arma::uvec & permutation) const {
    ...
    const arma::uvec new_table = torque::util::generate_index_table(
        new_dimension);
    ...
    for (arma::uword i = 0; i < total_elem; i++) {
      ...
      const arma::uvec old_indices = torque::util::index_to_indices(i,
                                                                    this->index_table,
                                                                    sort_index);
    ...
    return SparseTensor<T>(std::move(new_data),
                           gpu::util::arma_to_thrust_device<int32_t>(
                               new_indices), new_table, new_dimension);
