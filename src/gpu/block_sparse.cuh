...
//            cudaStreamCreate(&stream2);
    ...
    for (arma::uword non_trivial_i = 0;
      ...
      if (result_rank > 0) {
        ...
        if constexpr(std::is_same<T, float>::value) {
          cublasCheck(
              cublasSgemmStridedBatched(handle, CUBLAS_OP_N, CUBLAS_OP_T,
                                        A_leading_dim,
                                        B_leading_dim, contracting_n_elem, &one,
                                        A_ptr,
        ...
        if constexpr(std::is_same<T, float>::value) {
          cublasSdot(handle, arma::prod(padded_A_block_max_dimension), A_ptr, 1,
                     B_ptr, 1, dot_temp);
        } else if constexpr(std::is_same<T, double>::value) {
          cublasDdot(handle, arma::prod(padded_A_block_max_dimension), A_ptr, 1,
                     B_ptr, 1, dot_temp);
        } else if constexpr(std::is_same<T, half>::value) {
          T one = 1;
          T zero = 0;
          ...
          cublasHgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, 1, 1,
                      ...
                      arma::prod(padded_B_block_max_dimension), &zero, dot_temp,
                      1);
