...
#pragma acc kernels
    {
      for (arma::uword non_trivial_i = 0;
        ...
        const auto this_table = arma::conv_to<std::vector<int64_t>>::from(
            torque::util::generate_index_table(padded_A_block_max_dimension));
        const auto that_table = arma::conv_to<std::vector<int64_t>>::from(
            torque::util::generate_index_table(padded_B_block_max_dimension));

        const arma::uvec dot_dim = arma::join_vert(
            dimension_after_multiplication, arma::uvec{n_subblocks});
        const auto result_dim = arma::conv_to<std::vector<int64_t>>::from(
            dot_dim);
        const auto result_table = arma::conv_to<std::vector<int64_t>>::from(
            torque::util::generate_index_table(dot_dim));
